...
Servo  servo_1;  // servocontrolador (podem existir vários)

int  servo_pin1  =  3 ;  // Pino PWM para servo controle 
int  pos1  =  90 ;     // posição inicial do servo

Servo  servo_2;  // servocontrolador (podem existir vários)

int  servo_pin2  =  4 ;  // Pino PWM para servo controle 
int  pos2  =  90 ;     // posição inicial do servo

int setpoint_X = 0 ; //Valor onde o eixo X está em equilíbrio 
int setpoint_Y = 0 ; //Valor onde o eixo Y está em equilíbrio 
...
void setup()
{
  ...
  servo_1.attach(servo_pin1);  // inicia o controle do servo 
  servo_2.attach(servo_pin2);
  Serial.begin( 9600 );  // inicia o monitor serial 
  servo_1.write(pos1);  // move o servo 1 para 90 graus 
  servo_2.write(pos2);  // move o servo 2 para 90 graus 
  Serial.println( "Posicionados a 90 graus" ); 
}
  ...
  if (girX  > setpoint_X)
    {
      pos1 = pos1 + 2;
      servo_1.write(pos1);
    //  delay(20);  // atraso para velocidade máxima 
    }
  if (girX  < setpoint_X)
    {
     pos1 = pos1 - 2;
